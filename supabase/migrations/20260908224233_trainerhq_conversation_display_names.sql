-- Add member display names to the already-authorized conversation projection.
-- No profile health/goals or raw account IDs are exposed in user-facing labels.
...
begin
 ...
 when 'messages.threads' then
  select coalesce(jsonb_agg(to_jsonb(q)),'[]') into result from (select c.*,
   (select coalesce(jsonb_agg(jsonb_build_object('user_id',m.user_id,'member_role',m.member_role,'display_name',case when m.member_role='trainer' then (select p.display_name from public.trainer_profiles p where p.user_id=m.user_id) else (select coalesce(p.name,'WiFit client') from public.profiles p where p.id=m.user_id) end,'state',m.state,'read_at',m.read_at,'last_read_sequence',m.last_read_sequence)),'[]')
