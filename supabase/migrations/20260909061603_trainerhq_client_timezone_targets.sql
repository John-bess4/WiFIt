...
begin
 ...
 foreach item slice 1 in array array[
  array['where user_id=rel.client_id and effective_from<=current_date','where user_id=rel.client_id and effective_from<=(now() at time zone coalesce((select cp.timezone from public.client_tracking_preferences cp where cp.user_id=rel.client_id),''UTC''))::date'],
  array['where user_id=actor and effective_from<=current_date','where user_id=actor and effective_from<=(now() at time zone coalesce((select cp.timezone from public.client_tracking_preferences cp where cp.user_id=actor),''UTC''))::date']
 ] loop
