...
begin
 ...
 if result->'categories'->'diet'->>'state'<>'not_configured' or result->>'overall_score' is not null then raise exception 'Unconfigured scored zero';end if;n:=n+1;
 ...
 result:=trainerhq_private.adherence_day(t,r,day_value);
 if result->'categories'->'diet'->>'state'<>'incomplete' or (result->'categories'->'diet'->>'score')::numeric<>0 then raise exception 'Missing eligible log is not incomplete';end if;n:=n+1;
 if result->>'overall_state'<>'insufficient_shared_data' or (result->>'coverage')::int<>1 then raise exception 'Insufficient coverage hidden';end if;n:=n+1;
 ...
 result:=trainerhq_private.adherence_day(t,r,day_value);
 if (result->'categories'->'diet'->>'score')::numeric<>1 then raise exception 'Exact goals not full score';end if;n:=n+1;
 ...
 if trainerhq_private.range_score(1900,2000,0.1)<>1 or trainerhq_private.range_score(150,100,0,true)<>1 or trainerhq_private.range_score(50,100,0,true)<>0.5 then raise exception 'Range/minimum scoring wrong';end if;n:=n+1;
 update public.food_log set per100_cal=0,per100_protein=100,per100_carbs=0,per100_fat=0 where user_id=c;
 result:=trainerhq_private.adherence_day(t,r,day_value);
 if (result->'categories'->'diet'->>'score')::numeric<>0.3 then raise exception 'Diet weights wrong';end if;n:=n+1;
 ...
 result:=trainerhq_private.adherence_day(t,r,day_value);
 if (result->>'coverage')::int<>2 or (result->>'overall_score')::numeric<>0.65 then raise exception 'Eligible categories not renormalized';end if;n:=n+1;
 ...
 if result->'categories'->'diet'->>'state'<>'pending' or result->'categories'->'diet'->>'score' is not null then raise exception 'Before-cutoff missing score';end if;n:=n+1;
 update public.trainer_client_permissions set granted=false where relationship_id=r and scope='nutrition_adherence';
 result:=trainerhq_private.adherence_day(t,r,day_value);
 if result->'categories'->'diet'->>'state'<>'not_shared' or result->'categories'->'diet'->>'score' is not null or result->>'overall_score' is not null then raise exception 'Unshared contribution retained';end if;n:=n+1;
