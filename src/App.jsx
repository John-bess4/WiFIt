...
import { COLORS, SEED, LOCAL_FOOD_DB, SUPP_DB, SUPP_CATEGORY_DOTS, toSuppCategory, GOAL_OZ, CF_MAX_SERVING_G, MAX_FOOD_GRAMS, EXERCISE_LIBRARY, INITIAL_WORKOUTS, GOAL_RATES, calcCalFromRate, ACTIVITY, ACTIVITY_MULTS_BY_ID } from "./lib/constants.js";
...
function AISidePanel({open,onClose,onAddFood,onAddSupp,onAddWorkout,onAddWater,liveContext={},userName="",userId=""}){
  ...
  const callClaude=(userMsg,history)=>callCoach(userMsg,history,{liveContext,userName});
...
"Keep it personal and concise. Do NOT use any logging format — just plain text.";
  ...
  const applyActions=(parsed)=>{
    const {messages:out,hasSupp}=applyCoachActions(parsed,{onAddWater,onAddFood,onAddSupp});
    setMessages(prev=>[...prev,...out]);
    if(hasSupp&&parsed.msg)generateSuggestions(parsed.msg);
  };
